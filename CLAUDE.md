...
6. **Meilenstein 6 – Tabs/Layout-Verwaltung:** Tabs (oberhalb der Gruppen, unter der Suche) und das Gruppen-Layout über die UI verwalten (entspr. `settings.yaml` `layout`; vgl. `components/tab.jsx`, `components/services/group.jsx`).
   ...
   - **6b (geplant) – Per-Gruppe-Anzeige & Gruppen-Anordnung über die UI:**
     - **Services-Ausrichtung pro Gruppe:** `style: row` → Services **nebeneinander** (links→rechts, Grid) vs. Default → Services **untereinander** gestapelt (`flex flex-col`); `columns: N` = Services pro Reihe (via `columnMap`).
     - **Gruppen nebeneinander/untereinander:** wie viele Gruppen **max. nebeneinander** stehen dürfen (z. B. 3) über das globale `maxGroupColumns` (+ `fiveColumns`); `style: row`-Gruppen nehmen volle Breite.
     - Pro Gruppe zusätzlich: `header` (an/aus), `initiallyCollapsed`, `useEqualHeights`.
     - UI je Gruppe in `/admin/layout`; schreibt nach `settings.yaml` `layout[group]` (eemeli, kommentarerhaltend). Reorder von Tabs/Gruppen via Hoch/Runter bzw. 5d-Drag&Drop.
