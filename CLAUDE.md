...
7. **Meilenstein 7 – Authentifizierung & Rollen/Berechtigungen (umgesetzt):** Echte Cookie-Session (`iron-session`) statt statischem Token; voller Login-Zwang auch für das Dashboard; rollenbasierte Rechte v1.
   ...
   - **★ Vorgemerkt (M7b – User-Management-UI):** Admin-Bereich (z. B. `/admin/users`), um weitere Nutzer **über die UI** anzulegen/zu bearbeiten/zu löschen, Rolle (admin/viewer) zu setzen und Passwörter zu vergeben/zurückzusetzen — statt manuell `users.yaml` zu editieren. Nur für Admins (server-seitig erzwungen). Nutzt die vorhandenen Helfer in `src/utils/config/users.js` (`addUser`, scrypt via `password.js`); zusätzlich `updateUser`/`deleteUser`/`setPassword` ergänzen, neue Admin-only-API `/api/users` (GET/POST/PATCH/DELETE). **Leitplanken:** `users.yaml` bleibt **nicht** in `EDITABLE_CONFIGS` (kein Raw-Editor-Zugriff auf Hashes); Passwörter nie im Klartext zurückgeben/loggen; letzten Admin nicht löschbar/degradierbar machen. Knüpft direkt an M7 an.
...
8. **Meilenstein 8 – Theming, Branding & Custom UI (umgesetzt):** Neue Admin-Seite `/admin/theme` — eigenständige Seite (kein ConfigEditor-Shell), lädt `settings.yaml` + `custom.css` parallel, speichert getrennt.
   ...
   - **★ Vorgemerkt (8g – Presets entschärfen):** Die **benannten Presets** (8a, oben im Theme-Editor) nutzen noch teils kräftige Farben (z. B. „Homelab Neon", „Cherry Dark", „Dracula"). Auf Wunsch auf **gedämpfte Varianten** umstellen bzw. um ruhige Pastell-Presets ergänzen (analog zur kuratierten Farbpalette). Reine Daten-Änderung in `utils/config/theme-presets.js` (`THEME_PRESETS`) + ggf. neue gedämpfte Farben.
