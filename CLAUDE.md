...
12. **M12 – Favoriten (umgesetzt, v1, pro Nutzer):** Schnellzugriff mit **Anpinnen** (Stern pro Dienst), **★ Favoriten**-Sektion oben und **„Favoriten"-Filter** neben „All/Problematic". **Pro Nutzer, serverseitig** (nicht localStorage): neuer Per-User-Store `utils/config/user-preferences.js` schreibt **`CONF_DIR/user-preferences.json`** (keyed nach Username, atomarer Write, gekappt) — **getrennt von den YAML-Configs**, nicht in `EDITABLE_CONFIGS`. Session-gebundene API `/api/user/preferences` (`GET`/`PATCH toggleFavorite`) wirkt **immer nur** auf den eingeloggten User (Admin **und** Viewer für ihre eigenen Daten). Client-Hook `use-favorites.js` (SWR + optimistisch); geteilter `serviceKey(group,name)` = `` `${group}::${name}` `` (extrahiert nach `utils/services/service-key.js`, Problematic-Filter re-exportiert); pure Helfer `quick-access.js` (`buildFavoritesGroup`/`filterServiceGroupForFavorites`) bauen synthetische `ServicesGroup`s, deren Dienste `favoriteKey` (Original-Gruppe) tragen. `item.jsx`: Pin-Stern (immer sichtbar); `index.jsx`: Sektion + Filter (angezeigt, sobald ≥1 Favorit existiert). **v1-Grenzen:** Bookmarks nicht anpinnbar, keine geräteübergreifende Realtime-Sync (aber pro Account, da serverseitig). **Nachtrag (2026-07-04):** „Zuletzt/Häufig verwendet" wieder **entfernt** (auf Nutzerwunsch — störte mehr als es half). Damit fielen `buildRecentGroup`/`buildFrequentGroup`, die `usage`-Zeitreihe im Store, die `recordOpen`-Aktion (Store/API/Hook) und das Klick-Tracking in `item.jsx` weg. **Nachtrag 2 (2026-07-04):** der **„Quick access an/aus"-Schalter entfällt** — der Pin-Stern ist jetzt **immer** sichtbar (nicht mehr an `enabled` gekoppelt), Favoriten-Sektion/-Filter erscheinen automatisch bei mindestens einem Favoriten. Hintergrund: der Schalter versteckte früher (zusammen mit Recent/Frequent) auch die Sterne; wer ihn ausgeschaltet hatte, sah nach der Recent/Frequent-Entfernung keine Sterne mehr. `enabled`/`setEnabled` fielen aus Store/API/Hook weg; der Store ignoriert alte `usage`/`enabled`-Keys in `user-preferences.json` und lässt sie beim nächsten Write fallen (Favoriten bleiben erhalten). (F11)
13. **M13 – Kontext-Badges pro Dienst (P2) (umgesetzt, v1):** neues optionales, flaches Listen-Feld `badges: [lan, critical, …]` pro Service (analog `access.groups`, aber ohne `access`-Wrapper). Anzeige als kleine farbige Pills in einer **eigenen Zeile unter Name/Description** über die neue Komponente `src/components/services/service-badges.jsx` — verdrängt nicht die Status-Icons oben rechts. **Kuratierte Registry + freier Text:** neue `src/utils/config/badge-registry.js` (`BADGE_TYPES` = lan/vpn/public/admin/family/critical/backup/beta mit festem Label + literalen Tailwind-Klassen, `BADGE_BY_ID`, `NEUTRAL_BADGE_CLASS`); unbekannte/eigene ids sind **erlaubt** und rendern im neutralen Stil (kein Health-Warning). Schreibpfade: `applyBadges`/`SERVICE_DOCS_KEYS`-Muster in `yaml-edit.js` (Edit, reuse `normalizeAccessGroups`, leere Liste → Key weg) und `appendBadges` in `yaml-insert.js` (Neuanlage, inline-flow `badges: [a, b]`). Admin-Formular in `/admin/config`: **Checkbox-Grid** über die kuratierten Badges **plus** ein Komma-Freitextfeld für eigene — `form.badges` (Komma-String) ist die einzige Wahrheit, Checkboxen spiegeln/toggeln kuratierte ids darin, eigene leben im Textfeld. Card-Preview zeigt die Badge-Chips (dieselbe Registry). `badges` fließt per Spread durch `service-helpers.js` (keine Whitelist-Änderung); reiner Freitext, keine Redaction. **Wichtig (Tailwind v4):** die Badge-Farbklassen stehen **literal** in `badge-registry.js`, damit der Scanner sie nicht purged. **v1-Grenzen:** keine i18n für Badge-Labels (statische Registry-Labels), keine eigenen Farben für Custom-Badges (neutral), keine Icons in den Badges, keine automatische Ableitung (z.B. „public" nicht auto aus `urls`), keine Suche/Filter nach Badges. (F12)
...
15. **M15 – Service-Doku in der Kachel (P2) (umgesetzt, v1):** neues optionales, verschachteltes Feld `docs: { purpose, location, backup, admin, note, troubleshooting }` pro Service (analog `urls:` aus M14). Anzeige über ein neues `MdInfoOutline`-Info-Icon in `.service-tags` (nur sichtbar, wenn mindestens ein Feld gesetzt ist), das einen Headlessui-**Modal-Dialog** öffnet (`src/components/services/service-docs-button.jsx`, Vorbild `qr-button.jsx`) — bewusst kein `<details>`-Aufklapper, damit mehrzeiliger Troubleshooting-Text keine Nachbarkarten im Grid verschiebt. Das Feld "Server" aus der ursprünglichen Roadmap-Formulierung heißt **`location`** ("Standort"), um Verwechslung mit dem bereits bestehenden Top-Level-Feld `server:` (Docker/Proxmox-Stats-Bindung) zu vermeiden. Schreibpfade: `applyDocs`/`SERVICE_DOCS_KEYS` in `yaml-edit.js` (Edit, kommentarerhaltend über die eemeli-`yaml`-Document-API) und `appendDocs`/`DOCS_CONTEXT_KEYS` in `yaml-insert.js` (Neuanlage, Text-Splicing). Admin-Formular in `/admin/config` bekommt eine neue „Service docs (optional)"-Sektion (gleiches `<details>`-Muster wie „Network URLs") mit `DOCS_FORM_FIELDS`; `note`/`troubleshooting` als `<textarea>`, die übrigen vier als `<input>`. Card-Preview zeigt zusätzlich einen kleinen, nicht-interaktiven Info-Chip, wenn ein Service `docs` hat. `docs.*` sind reiner Freitext ohne Secret-Überschneidung, keine Änderung an `secret-mask.js` nötig; `service-helpers.js` reicht das Feld bereits ungefiltert per Spread durch. **Härtungsfund während der Implementierung:** `quoteScalar`/`needsQuoting` in `yaml-insert.js` erkannten eingebettete Zeilenumbrüche nicht als quoting-pflichtig und escapten sie auch nicht — ein mehrzeiliger `troubleshooting`-Wert beim **Neuanlegen** eines Service hätte die YAML-Datei gebrochen (unquotierter Skalar mit rohem Zeilenumbruch mitten in der Textzeile). Fix: `needsQuoting` erkennt `[\r\n]` jetzt als quoting-pflichtig, `quoteScalar` escaped Zeilenumbrüche zur Zwei-Zeichen-Sequenz `\n`; per Regressionstest abgesichert. **v1-Grenzen:** kein Markdown-Rendering (nur `whitespace-pre-wrap`), kein Rich-Text-Editor, keine Anhänge/Bilder, kein Auto-Linking von URLs im Text, kein Zeichenlimit (kein Präzedenzfall im Projekt dafür), Doku-Inhalte nicht Teil der Quick-Launch-Suche (M11). (F17)
...
#### Härtung, Konsolidierung & Ballast-Abbau (2026-07-06)

Reine Aufräum-/Härtungsrunde (kein Feature). **Ballast entfernt:** der versehentlich hier eingecheckte Fremd-Klon `codex-desktop-linux/` (2,4 GB, Codex-Desktop-App/Rust, verwaister Gitlink im Index, nie in HEAD) ist physisch gelöscht **und** aus dem Index genommen (bleibt via `.gitignore` draußen); die abgeschlossenen Arbeits-Docs `AGENTS.md`, `HANDOFF.md`, `Widgets_Codex_Auftrag.md` sind entfernt (einzige Referenz in `README.md` auf nur `CLAUDE.md` reduziert). Der `pnpm lint`-Scope bleibt bewusst `eslint src`.

- **Dependency-Härtung (`pnpm-workspace.yaml`-Overrides):** `pnpm audit` war auf **27 Funde (1 critical, 15 high, 8 moderate, 3 low)** gedriftet — die frühere „0 critical / 9 Funde"-Notiz oben ist damit **überholt**. Kritisch war `i18next-fs-backend@2.6.4 < 2.6.6` (Prototype Pollution, transitiv via `next-i18next`). Neue/aktualisierte Overrides (nur **patch/minor innerhalb derselben Major**, lower-bounded, damit fremde Versionslinien unberührt bleiben): `i18next-fs-backend@^2.6.6`, `form-data@^4.0.6`, `js-yaml@^4.1.2` (direkte dep, jetzt 4.3.0), `ws@^8.21.0`, `undici@^7.28.0`, `@grpc/grpc-js@^1.14.4`, `protobufjs@^7.6.3`, `tar@^7.5.16`, `postcss@^8.5.10`, `minimatch@^3.1.4`/`^9.0.7`, `@eslint/plugin-kit@^0.3.4`. Ergebnis: **27 → 3 Funde, 0 critical**.
- **Bewusst offen (3 Funde, alle nicht-Runtime bzw. Major-Risiko):** `vite` (high, Windows-only Dev-Server-`fs.deny`-Bypass) — vitest pinnt vite als Peer auf 7.3.2, der Override greift dort nicht; vite ist **dev-only**, landet nie im Produktions-Image → akzeptiert. `uuid` (moderate) — Fix `>=11.1.1` würde transitive uuid@3/5/6-Konsumenten auf einen Major zwingen → nicht erzwungen. `launch-editor` (moderate, dev-only). `minimatch@10`/andere reine Dev-Ketten bleiben ebenfalls offen, wo nur ein Major fixt.
- **vitest-OOM behoben (stabiler Volltest):** `vitest.config.mjs` von `pool: "threads"` auf **`pool: "forks"` mit `poolOptions.forks.maxForks: 4`** umgestellt. Der geteilte Thread-Heap akkumulierte über die große jsdom-Suite Speicher und wurde OOM-gekillt; Forks geben den Heap pro Testfile-Prozess frei. Die Vollsuite läuft jetzt **komplett durch: 588 Dateien / 1959 Tests grün** (~243 s), kein OOM mehr.
- **Verifikation:** `pnpm test` **588/1959 grün** (ohne OOM), `pnpm build` grün, `pnpm lint` 0 Fehler, `pnpm audit` **0 critical / 3 Restfunde** (dokumentiert). 2,4 GB Plattenplatz frei.
...
### Verifikationsstatus (M15 Service-Doku in der Kachel v1, 2026-07-04)

`pnpm test` grün (**581 Testdateien / 1898 Tests**), `pnpm lint` 0 Fehler, `pnpm build` grün. Browser-E2E gegen `pnpm start` mit einer isolierten Test-Config (gleiches Vorgehen wie bei M10: temporärer `.claude/launch.json`-Eintrag, danach entfernt; reale `config/` unverändert, per `git status` bestätigt).

- **Neue Tests:** `yaml-edit.test.js` (+5: `docs`-Map anlegen/einzelnes Feld ändern/entfernen/ganze Map entfernen/mehrzeiliger Wert), `yaml-insert.test.js` (+4: `buildServiceEntry` mit `docs`, `insertService` mit Sonderzeichen, plus der `quoteScalar`-Regressionstest unten), neues `service-docs-button.test.jsx` (5 Tests), `item.test.jsx` (+2: Info-Button nur bei vorhandenen `docs`, korrekte Weitergabe), neues `admin-config-docs.test.jsx` (3 Tests: Add-Flow mit nur befüllten Feldern, kein `docs`-Key ohne Eingabe, Edit-Flow mit Prefill + nur geänderte Felder).
- **Browser-Beweis (`pnpm start` gegen isolierte Config, ein Admin):** `/admin/config` zeigt auf der Jellyfin-Karte einen `MdInfoOutline`-Info-Chip ("Hat Service-Doku"); der Edit-Dialog befüllt alle sechs Doku-Felder korrekt vor, inklusive mehrzeiligem `troubleshooting`-Text mit echtem Zeilenumbruch. Ändern von nur `backup` und Speichern schreibt ausschließlich dieses Feld neu (Disk-Diff bestätigt, andere Felder/Grafana-Eintrag byte-gleich). Neuanlage eines zweiten Service ("Uptime Kuma") mit mehrzeiligem `troubleshooting` über "+ Add Service" erzeugt gültiges, korrekt escaptes YAML (`\n` als Zwei-Zeichen-Sequenz), das Disk-File parst sauber und beide Services koexistieren.
- **Härtungsfund während der Implementierung (behoben):** `needsQuoting`/`quoteScalar` in `yaml-insert.js` erkannten eingebettete Zeilenumbrüche nicht als quoting-pflichtig und escapten sie auch nicht — ein mehrzeiliger `troubleshooting`-Wert beim Neuanlegen eines Service hätte die YAML-Datei gebrochen (unquotierter Skalar mit rohem Zeilenumbruch mitten in der Textzeile, `YAMLException` beim Re-Parse). Erst durch einen gezielt geschriebenen `insertService`-Test mit mehrzeiligem `docs`-Feld aufgedeckt. Fix: `needsQuoting` erkennt `[\r\n]` jetzt als quoting-pflichtig, `quoteScalar` escaped Zeilenumbrüche zur `\n`-Zwei-Zeichen-Sequenz; dedizierter Regressionstest in `yaml-insert.test.js` (`describe("quoteScalar")`).
- **Bekannte Einschränkung:** dieselbe SWR/Dashboard-Fetch-Eigenheit wie bei M10 (siehe oben) trat auch hier auf — die Dashboard-Verifikation erfolgte daher über `/admin/config` (Raw-YAML-Fetch-Pfad, nicht betroffen) statt über die Live-Dashboard-Kachel; die Kachel-Logik selbst (Info-Icon-Sichtbarkeit, Dialog-Inhalt) ist über `item.test.jsx`/`service-docs-button.test.jsx` vollständig abgedeckt.

### Verifikationsstatus (M13 Kontext-Badges pro Dienst v1, 2026-07-05)

`pnpm test` grün (**583 Testdateien / 1910 Tests**), `pnpm lint` 0 Fehler, `pnpm build` grün. Browser-E2E gegen `pnpm start` mit isolierter Test-Config (temporärer `.claude/launch.json`-Eintrag, danach entfernt; reale `config/` unverändert).

- **Neue Tests:** `badge-registry.test.js` (eindeutige ids, `className`s vorhanden, `BADGE_BY_ID` vollständig), neues `service-badges.test.jsx` (nichts bei leer, kuratiertes Registry-Label, Rohtext bei Custom, Mehrfach), `yaml-edit.test.js` (+1: `badges` anlegen/ändern/entfernen, dedupe), `yaml-insert.test.js` (+2: `buildServiceEntry` inline-flow + Weglassen bei leer), `item.test.jsx` (+2: Badge-Zeile nur bei `service.badges`), `admin-config-docs.test.jsx` (+2: Checkbox togglet id im Komma-String + Custom koexistiert; Prefill spiegelt Checkboxen).
- **Browser-Beweis:** `/admin/config`-Preview-Card zeigt für Jellyfin die Chips „LAN"/„Kritisch"; der Edit-Dialog befüllt Textfeld **und** Checkbox-Grid synchron vor; „Backup" per Checkbox + „mein-eigenes" per Freitext ergänzt und gespeichert → `services.yaml` enthält `badges: [lan, critical, backup, mein-eigenes]` (kuratiert + custom, Kommentare/andere Felder erhalten, per Disk-Diff bestätigt). Add-Flow (inline-flow-Schreibpfad) über Unit-Tests abgedeckt.
- **UI-Fix während der Verifikation:** die Badge-Sektion lag zunächst in einer `Field`-Komponente (die ihre Kinder in ein `<label>` wickelt) — dadurch verschluckte das äußere Label die Accessibility-Namen der Checkboxen (erste Checkbox bekam den kompletten Sektionstext als Namen). Behoben durch Ersetzen des `Field`-Wrappers durch einen normalen `<div>` + `<span>`-Überschrift (analog zu den Doku-`<details>`-Sektionen); das Textfeld hat ein eigenes `aria-label="Service badges"`.
- **Bekannte Einschränkung:** dieselbe SWR/Dashboard-Fetch-Eigenheit wie bei M10/M15 — die Badge-Zeile auf der Live-Kachel ist über `item.test.jsx`/`service-badges.test.jsx` abgedeckt, verifiziert wurde über `/admin/config`.
