...
12. **M12 – Favoriten (umgesetzt, v1, pro Nutzer):** Schnellzugriff mit **Anpinnen** (Stern pro Dienst), **★ Favoriten**-Sektion oben und **„Favoriten"-Filter** neben „All/Problematic". **Pro Nutzer, serverseitig** (nicht localStorage): neuer Per-User-Store `utils/config/user-preferences.js` schreibt **`CONF_DIR/user-preferences.json`** (keyed nach Username, atomarer Write, gekappt) — **getrennt von den YAML-Configs**, nicht in `EDITABLE_CONFIGS`. Session-gebundene API `/api/user/preferences` (`GET`/`PATCH toggleFavorite|enabled`) wirkt **immer nur** auf den eingeloggten User (Admin **und** Viewer für ihre eigenen Daten). Client-Hook `use-favorites.js` (SWR + optimistisch); geteilter `serviceKey(group,name)` = `` `${group}::${name}` `` (extrahiert nach `utils/services/service-key.js`, Problematic-Filter re-exportiert); pure Helfer `quick-access.js` (`buildFavoritesGroup`/`filterServiceGroupForFavorites`) bauen synthetische `ServicesGroup`s, deren Dienste `favoriteKey` (Original-Gruppe) tragen. `item.jsx`: Pin-Stern; `index.jsx`: Sektion + Filter + **An/Aus-Toggle**. **Abschaltbar** (aus → keine Sektion/Sterne). **v1-Grenzen:** Bookmarks nicht anpinnbar, keine geräteübergreifende Realtime-Sync (aber pro Account, da serverseitig). **Nachtrag (2026-07-04):** „Zuletzt/Häufig verwendet" wieder **entfernt** (auf Nutzerwunsch — störte mehr als es half). Damit fielen `buildRecentGroup`/`buildFrequentGroup`, die `usage`-Zeitreihe im Store, die `recordOpen`-Aktion (Store/API/Hook) und das Klick-Tracking in `item.jsx` weg. Der Store ignoriert einen ggf. noch vorhandenen `usage`-Key in `user-preferences.json` und lässt ihn beim nächsten Write fallen (Favoriten bleiben erhalten). (F11)
